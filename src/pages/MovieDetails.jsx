...
import transformSingleShowTimings from "../lib/transformSingleShowTimings";
...
const MovieDetails = () => {
  ...
  const getShow=async ()=>{
   axios.get('http://localhost:8080/shows/allShows/'+id)
        .then(response => {
            console.log(response.data);
            console.log(response.data.shows[0]);
            if(response.data.shows){
              setShows(response.data.shows[0]);
              setDateTime(transformSingleShowTimings(response.data.shows));
            }
  ...
  return !loading ?  (
    <div className="px-6 md:px-16 lg:px-40 pt-30 md:pt-50">
      <div className="flex flex-col md:flex-row gap-8 max-w-6xl mx-auto">
        ...
        <img src={shows.movie.imagesUrl} alt="" className="max-md:mx-auto rounded-xl h-104 
         ...
         <div className="relative flex flex-col gap-3">
          ...
          <div className="flex items-center gap-2 text-gray-300">
            ...
            {shows.movie.voteAverage} User Rating
          </div>
          ...
          <p className="">
            ...
            {new Date(shows.movie.createdAt).getFullYear()}
          </p>
       ...
       {/* Commenging below code because cast is not available do it later */}
